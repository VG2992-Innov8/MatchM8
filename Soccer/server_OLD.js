...
// Never allow admin.html to be shown inside an <iframe>
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.warn(
    "[WARN] Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Server will start but DB calls will fail."
  );
}
...
// -----------------------------
// Admin guard
//  - Accepts ADMIN_TOKEN (preferred)
//  - Also accepts SUPABASE_SERVICE_ROLE_KEY in dev to reduce friction
// -----------------------------
function requireAdmin(req, res, next) {
  const token = req.headers["x-admin-token"] || "";
  const ok =
    token &&
    (token === ADMIN_TOKEN || token === SUPABASE_SERVICE_ROLE_KEY);
  if (!ok) return res.status(401).send("Admin token required");
  next();
}
...
app.put("/predictions/upsert", requirePredToken, async (req, res) => {
  ...
  const { data: fixtures, error: fxErr } = await supabase
    .from("fixtures")
    .select("id, week, kickoff_at, league_id, home_team, away_team")
    .eq("week", week);
  ...
  await writeAudit({
    actor_type: "player",
    actor_id: player_id,
    action: "predictions_upsert",
    meta: { week, count: rows.length },
  });
  res.sendStatus(200);
});
...
app.get("/leaderboard/export.csv", requireAdmin, async (req, res) => {
  ...
  await writeAudit({
    actor_type: "admin",
    actor_id: null,
    action: "export_leaderboard_csv",
    meta: { week_to: weekTo, count: rows.length },
  });
});
...
// GET /audit/actions?limit=200&actor_type=&action=&from=&to=
app.get("/audit/actions", requireAdmin, async (req, res) => {
  const limit = Math.min(Number(req.query.limit) || 200, 1000);
  let q = supabase
    .from("audit_log")
    .select("id, created_at, actor_type, actor_id, action, meta")
    .order("created_at", { ascending: false })
    .limit(limit);

  if (req.query.actor_type) q = q.eq("actor_type", req.query.actor_type);
...
// GET /audit/export.csv?from=&to=&actor_type=&action=
app.get("/audit/export.csv", requireAdmin, async (req, res) => {
  let q = supabase
    .from("audit_log")
    .select("id, created_at, actor_type, actor_id, action, meta")
    .order("created_at", { ascending: false });
...
app.listen(PORT, () => {
  console.log(`MatchM8 server listening on http://localhost:${PORT}`);
});
